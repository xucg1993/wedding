    ...
    $.fn.extend({
        ...
        layoutH: function($refBox){
            return this.each(function(){
                ...
                if ($this.isTag("table")) {
                    //add by nowind
//                   var length = $this.find("tbody").length;
//                    if(length > 0) {
//                        iH -= 52 //* length;  //无contentTitle 12
//                    }

                    $this.removeAttr("layoutH").wrap('<div layoutH="'+iLayoutH+'" style="overflow:auto;height:'+iH+'px"></div>');
                } else {
                    //add by nowind
//                    var length = $this.find(".gridTbody > table").length;
//                    if(length > 0) {
//                        iH -= 63 //* length;  //无contentTitle 23
//                    }
...
(function($){
    ...
    $.fn.jBar = function(options){
        var op = $.extend({container:"#container", collapse:".collapse", toggleBut:".toggleCollapse_s span", sideBar0:"#navMenu", sideBar:"#sidebar", sideBar2:"#sidebar_s", splitBar:"#splitBar", splitBar2:"#splitBarProxy"}, options);
        return this.each(function(){
            var jbar = this;
            var sbar = $(op.sideBar2, jbar);
            var bar = $(op.sideBar, jbar);
            var bar0 = $(op.sideBar0);
            $(op.toggleBut, bar0).click(function(){
                if(bar.is(":hidden")) { return; }
                DWZ.ui.sbar = false;
                $(op.splitBar).hide();
...
function initLayout(){
    ...
    //$("#container .tabsPageContent").height(iContentH - 24).find("[layoutH]").layoutH();
    var tabsPageContent = $("#container .tabsPageContent");
    tabsPageContent.height(iContentH - 37).find("[layoutH]").layoutH();
    var tpcPFC = tabsPageContent.find("div.pageFormContent");
//    if( tpcPFC ){ alert(1);
//        tpcPFC.height(tpcPFC.height()-100); //by nowind
//    }
    $("#sidebar, #sidebar_s .collapse, #splitBar, #splitBarProxy").height(iContentH - 6);   //by nowind 原值：5
    //$("#sidebar_s .collapse").height( $(this).height()-5 ); //by nowind
...
function initUI(_box){
    ...
    if ($.fn.selectedToSort) $("a[target=selectedToSort]", $p).selectedToSort();
    if ($.fn.selectedToRow) $("a[target=selectedToRow]", $p).selectedToRow();
    if ($.fn.selectedToRows) $("a[target=selectedToRows]", $p).selectedToRows();
    ...
    $.fn.extend({
        ...
        tabs: function (options){
            ...
            function initTab(jT){
                ...
                if(jGroups.length < 2){return;} //by nowind
...
(function($){
    ...
    $.fn.extend({
        ...
        selectedToSort: function(){ //by nowind
            ...
            function _getValues(selectedIds, selectedVals, targetType){
                var reg = "^\d+$";
                var values = "";
                var $box = targetType == "dialog" ? $.pdialog.getCurrent() : navTab.getCurrentPanel();
                $box.find("input:checked").filter("[name='"+selectedIds+"']").each(function(i){
                    var valEle = $(this).parent().parent().parent().find("[name='"+selectedVals+"']"); //注意层级
                    if(typeof valEle != "undefined"){
                        var val = $.trim(valEle.val());
                        //if (!reg.test(val)){ alert(val); return ""; }
                        values += i==0 ? val : ","+val;
                    }
                });
                return values;
            ...
            return this.each(function(){
                var $this = $(this);
                var selectedIds = $this.attr("rel") || "ids";
                var selectedVals = $this.attr("name") || "sorts";
                ...
                $this.click(function(){
                    var targetType = $this.attr("targetType");
                    var ids = _getIds(selectedIds, targetType);
                    var values = _getValues(selectedIds, selectedVals, targetType);
                    if (!ids || !values) {
                        alertMsg.error($this.attr("warn") || DWZ.msg("alertSelectMsg"));
                        return false;
                    }
        ...
        //单行表单提交
        selectedToRow: function(){ //by nowind

            return this.each(function(){
                var $this = $(this);
                var rowid = $this.attr("rel");    // || "rel";
                var postType = $this.attr("postType");  // || "map";
                ...
                $this.click(function(){
                    ...
                    var vals = "";
                    var $row = $("#" + rowid);
                    var relVal = $this.attr("name");
                    $row.find("[rel='"+relVal+"']").each(function(i){
                    ...
                    function _doPost(){
                        $.ajax({
                            ...
                            data: function(){
                                if (postType == 'map'){
                                    return $.map(vals.split(','), function(vals, i) {
                                        return {name:vals.split(':')[0], value: vals.split(':')[1]}
                                    })
                                } else {
                                    var _data = {};
                                    _data["values"] = vals;
        ...
        //多行选中表单提交
        selectedToRows: function(){ //by nowind

            function _getIds(selectedIds, targetType){  //value
                var vals = "";
                var $box = targetType == "dialog" ? $.pdialog.getCurrent() : navTab.getCurrentPanel();
                $box.find("input:checked").filter("[name='"+selectedIds+"']").each(function(i){
                    var $row = $("#row" + $(this).val());
                    var itemVal = "";
                    $row.find("[rel='"+selectedIds+"']").each(function(j){
                        itemVal += j==0 ? $(this).val() : ","+$(this).val();
                    });
                    vals += i==0 ? itemVal : ";"+itemVal;
                });
                return vals;
            }
            return this.each(function(){
    ...
    var _onchange = function (event){
        ...
        $.ajax({
            ...
            success: function(json){
                ...
                html += '<option value="0">选择</option>';   //by nowind
...
(function($){
    //清除机构输入表单
    clearOrganameAndId = function(idPrefix){
        $("input[name='"+idPrefix+".organame']", navTab.getCurrentPanel()).val("");
        $("input[name='"+idPrefix+".orgaid']", navTab.getCurrentPanel()).val("");
    }
})(jQuery);
...
(function($){
    $.extend({
        //弹出窗口中执行并关闭、刷新当前navTab
        execBack: function(urlAndParams, callback){
            $.ajax({url: urlAndParams,
                type: "POST",
                dataType:"json",
                success: function(json){
...
(function($) {
    $.fn.extend({
        //文本框内提示文字根据内容显示或隐藏
        inputTextDes: function(desText) {
            $(this).bind("click", function(){ var text = $(this).val().trim(); if(text == desText){ $(this).val(""); } });
            $(this).bind("blur", function(){ var text = $(this).val().trim(); if(text == ""){ $(this).val(desText); } });
        },
        //当前位置插入文本
        insertContent: function(text, t) {
            ...
            if (document.selection) { //ie
                ...
                if (arguments.length == 2) {
                    ...
                    t <= 0 ? sel.moveStart("character", wee - 2 * t - text.length) : sel.moveStart("character", wee - t - text.length);
                    sel.select();
                }
